...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        const main = async () => {
            ...
            if (last_post_id === 0) {
                const count = await prisma.$queryRaw(`SELECT COUNT(*) FROM "users" JOIN "posts" on posts.user_id = users.id WHERE posts.active = 0 AND posts.verify = 1`)
                const answer = await prisma.$queryRaw(`SELECT posts.id, posts.title, posts.description, posts.price, posts.category_id, posts.photo, posts.user_id, users.name, users."userPhoto" FROM "users" JOIN "posts" on posts.user_id = users.id WHERE posts.active = 0 AND posts.verify = 1 AND posts.id > ${last_post_id} ORDER BY posts.id LIMIT ${page_limit}`)
