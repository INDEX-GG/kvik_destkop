...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        ...
        const main = async () => {
            ...
            let region = req.body.region.toLowerCase()
            const post  = await pool.query(`SELECT "posts".subcategory, "posts".category_id FROM "posts" INNER JOIN "users" ON posts.user_id = users.id WHERE posts.id = $1`, [post_id])
            ...
            if (filterValue !== undefined) {
                for (let variable of filterValue) {
                    let bodyVariable  = req.body[variable]
                ...
                const answer  = await pool.query(`SELECT * FROM "posts" WHERE (LOWER (posts.category_id) LIKE $1) AND posts.active = 0 AND posts.verify = 0 AND LOWER (city) LIKE $2 LIMIT $3`, [full_category + '%', region + '%', 24])
                return(answer.rows)
            }
        }
        try {
