...
const CustomButtonUI = ({
  ...
  const classesButton = customRoot
    ? `${customRoot} ${classes.button}`
    : classes.button;
  const classesCircle = customCircle
    ? `${customCircle} ${classes.circle}`
    : classes.circle;
  const classesRoot =
    type === "circle" ? `${classesButton} ${classesCircle}` : classesButton;
