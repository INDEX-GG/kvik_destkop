...
import trucks from "../../../public/subcategories/tractors_data.json";
import { OnlyNumbersMask } from "../../../lib/onlyNumbersMask";
...
export default function Tractors({ data }) {
  const trucksData = trucks["tractors"];
  ...
  const bodyType = useMemo(() => {
    methods.setValue("bodytype", modelData?.[0].value);
  ...
  return (
    <>
      ...
      {make && model && (
        <Box className={classes.formElem}>
          <Typography className={classes.formTitleField}>Тип кузова</Typography>
          <Box className={classes.formInputField}>
            <Controller
              shouldUnregister
      ...
      {data?.slice(2).map((item, idx) => {
        if (item.alias === "truck_enginesize") {
          switch (item.type) {
            case "text":
              return (
                <Box className={classes.formElem}>
