...
console.log(activeOffersBox)
  ...
  const navItems = [
    ...
    { id: 3, title: "Архив", content: <Archive offers={dataArhive} />, count: archiveOffersBox?.length },
  ];
  ...
  return (
    <>
      ...
      {navItems.map((item) => itemNav.i === item.id && (OffersBox.length > 0 ? item.content : <Placeholder key={item.id}/>))}
    </>
  );
};
