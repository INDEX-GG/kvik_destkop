...
const useStyles = makeStyles((theme) => ({
  ...
  offer__image: {
    background:
      "no-repeat center center url(../../../icons/photocard_placeholder.svg)",
  ...
  return (
    <>
      ...
      >
        <div>
          {offersLength > 1 && (
            <div className={classes.offer__pub__check}>
              <Checkbox
        ...
        <div
          className={`${classes.description} ${
            isArchive ? classes.description_archive : ""
          } ${isWaith ? classes.description_waith : ""}`}
        >
          <div
            className={`${classes.top} ${
              isArchive ? classes.top_archive : ""
            } ${isWaith ? classes.top_waith : ""}`}
          >
            <div
              className={`${classes.column} ${
                isArchive ? classes.left_archive : ""
              } ${isWaith ? classes.left_waith : ""}`}
            >
              <div className={!mobileBanned ? classes.left__info : ""}>
                <p className={`${classes.main__text} ${classes.mobile__font}`}>
                  {ToRubles(offer.price)}
                </p>
                <p
                  className={`${classes.main__text} ${
                    isWaith ? classes.main_text__waint : ""
                  }`}
                >
                  {ellipsis(offer.title, titleWidth)}
                </p>
                {isArchive && (
                  <div
                    className={`${classes.left__date} ${
                      isWaith || isArchive ? classes.left__date__wait : ""
                    }`}
                  >
                    <p
                      className={`${classes.main__text} ${
                        classes.lignt__text
                      } ${isWaith || isArchive ? classes.bottom__wait : ""}`}
                    >
                      {!screenIsMobile ? "Дата последнего редактирования " : ""}
                      {ToFullDate(offer.archived_time)}
                    </p>
                  </div>
                )}
              </div>
              {isActive && (
                ...
                >
                  <p
                    className={`${classes.main__text} ${classes.lignt__text} ${
                      isWaith || isArchive ? classes.bottom__wait : ""
                    }`}
                  >
                    Дата публикации {ToFullDate(offer.created_at)}
                  </p>
                  {isActive && (
                    <p
                      className={`${classes.main__text} ${
                        classes.position__absolute
                      }`}
                    >
              ...
              {isBanned && (
                <div>
                  <p className={classes.text__banned}>
                    Причина отклонения:{" "}
                    <span>
                      {ellipsis(
                        moderatorMessages.join(" / "),
                        screenIsMobile ? 50 : 250
                      )}
                    </span>
                  </p>
                </div>
              ...
              {isWaith && (
                ...
                >
                  ...
                  >
                    Дата последнего редактирования 00.00.0000 00:00
                  </p>
                  {isActive && (
                    <p
                      className={`${classes.main__text} ${
            ...
            <div className={classes.column}>
              <div
                className={`${classes.column} ${classes.end} ${
                  classes.mobile__width
                } `}
              >
                <BtnActive
                  isActive={isActive}
