...
const PasswordValidationBox = ({
  ...
  const lengthClass = clsx(
    classes.constraint,
    validationResults?.length !== undefined &&
      (validationResults?.length ? classes.valid : classes.invalid)
  );
  const numberClass = clsx(
    classes.constraint,
    validationResults?.number !== undefined &&
      (validationResults?.number ? classes.valid : classes.invalid)
  );
  const letterClass = clsx(
    classes.constraint,
    validationResults?.letter !== undefined &&
      (validationResults?.letter ? classes.valid : classes.invalid)
  );
  ...
  return (
    <p className={blockClass} {...paragraphProps}>
      Придумайте пароль от <span className={lengthClass}>8 знаков</span> из{" "}
      <span className={numberClass}>цифр</span> и{" "}
      <span className={letterClass}>латинских букв</span>
    </p>
  );
};
...
export const PasswordForm = () => {
  ...
  const valBoxClass = clsx(
    classes.validContainer,
    (validationResults || isValidationVisible) && classes.visible
  );
  ...
  const handlerPasswordChange = async (formData) => {
    const [isValidPassword, results] = validatePassword(
    ...
    try {
      const resultUpdate = await updatePasswordOld(
        formData.old_password,
        formData.password,
        token
      );
  ...
  return (
    ...
    >
      <div className="form__section form__section--password">
        <label className="form__label" htmlFor="user-current-pass">
          Текущий пароль
        </label>
        <div className="form__content">
          ...
          />
          <button className="form__button" onClick={handlerPasswordVisiblity} />
        </div>
      </div>

      <div className="form__section form__section--password">
        <label className="form__label" htmlFor="user-new-pass">
          Новый пароль
        </label>
        <div className="form__content">
          <input
...
export const PasswordFormMobile = () => {
   ...
   */
  const handlerPasswordChange = async (formData) => {
    const [isValidPassword, validResults] = validatePassword(
      formData.old_password,
      formData.password
    );
  ...
  return (
    ...
    >
      <div
        className={`form__section form__section--password ${classes.section}`}
      >
        <div className="form__content">
          <input
            {...register("old_password", {
          ...
          />
          <button
            className={`form__button ${classes.eye}`}
            onClick={handlerPasswordVisiblity}
          />
        </div>
      </div>
