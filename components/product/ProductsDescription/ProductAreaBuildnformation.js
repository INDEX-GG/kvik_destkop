...
const ProductAreaBuildnformation = ({data, mobile, description, productData}) => {
    ...
    useEffect(() => {
        ...
        if (productData) {
            setProducts([...productData?.filter(el => el.alias !== "communications_checkbox"),
                {alias: "desc"},
                ...productData?.filter(el => el.alias === "communications_checkbox")])
        }
                                ...
                                if (el.alias === "distance_to_city") return (
                                    <div className="productWrap" key={i} style={{border: "none"}}>
                                        ...
                                        <pre className='productDescription'
                                             style={{margin: "14px 0"}}>{data[el.alias]} км.</pre>
                                    </div>
                                )
