...
const ProductBuildInformation = ({data, mobile, description, productData}) => {
  ...
  const [hide, setHide] = useState(true)
    ...
    if (productData){
      setProducts([...productData?.filter(el => el.alias !=="accommodations" && el.alias !=="facilities" && el.alias !=="infrastructure"),
      {alias: "desc"},
      ...productData?.filter(el => el.alias ==="accommodations" || el.alias ==="facilities" || el.alias ==="infrastructure")])
    }
