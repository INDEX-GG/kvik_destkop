...
//  Формирование 0 для даты, если она меньше 10. 09 / 08 / 07
...
//  Генерация даты для выбранного switch / case
...
export const formDefaultValue = (query, methods) => {
  if (query) {
    ...
    if (query?.bodytype) {
      query.bodytype = Array.isArray(query?.bodytype) ? query?.bodytype : [query?.bodytype]
    }
