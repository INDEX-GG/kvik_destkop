...
export const getDataByPost = async (url, data) => {
	const res = await axios.post(`${BASE_URL}${url}`, data)
		.then((r => r.data))
		.catch(e => {
			console.error('Ошибка запроса getDataByPost ', e)
			throw e
		})
	return await res;
}
...
export const getTokenDataByPost = async (url, data, token) => {
	const sendToken = token ? token : await getDataByGet('/api/refresh')
	const res = await axios.post(url, data, {
		headers: {
			"x-access-token": sendToken.authToken || sendToken
		}
	})
		.then(r => r.data)
...
export const postLikeCommentArray = async (id, token, arrayUnlike = [], arrayLike = []) => {
		const data = await getTokenDataByPost(`/api/likePosts`, {
		user_id: id,
		like_posts: arrayLike,
		unlike_posts: arrayUnlike,
	}, token)
		.then(r => r.data)
		.catch(e => console.error('Ошибка получения лайков', e))
	return await data;
 ...
 * @returns { Promise<{ message: string }> } "successfully update"
 */
 export const updatePasswordOld = async (oldPassword, password, token) => {
	const old_pass = SecretPassword({ password: oldPassword });
	const new_pass = SecretPassword({ password: password });

	const objPasswords = {
		old_pass: old_pass.password,
		new_pass: new_pass.password
	}
