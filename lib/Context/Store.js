...
const StoreProvider = ({ children }) => {
			...
			await getDataByPost('/api/getUser?new', { id: id })
				.then(r => {
					...
					if (r !== undefined) {
						...
						if (typeof r.subscriptions === 'string') {
							r.subscriptions = JSON.parse(r.subscriptions)
						}
						setUserInfo(r);
					}
				})
	...
	const setLikeCommentArray = (array) => {
		postLikeCommentArray(id, array)
		array.map( (item ) =>{
			setUserInfo(p => {
				p.favorites = checkTwin(p.favorites, item.id)
				return {
					...p,
					favorites: p.favorites.length > 0 ? [...p.favorites, { post_id: item.id, comment: item.comment, condition: item.like }] : [{ post_id: item.id, comment: item.comment, condition: item.like }]
				}
			})
		})
	}
