...
const fetch = async (data, setState, id, token) => {
    ...
    if(likePosts.like.length || likePosts.unLike.length) {
        const x = likesParser(likePosts)
        const isEmpties = x.like_posts.length === 0 && x.unlike_posts.length === 0
        x.user_id = id

        isEmpties ? null : await getTokenDataByPost('/api/likePosts', x, token)
...
// запускаем интервал, для оптравки данных на бек.
    ...
    // useEffect(() => {
    //     window.addEventListener('click', (() => console.log('kek')))
    // }, [])
	...
	return (
		<StatisticsCTX.Provider value={{addView, addContactClick, addLike, addUnLike, addSubscribers, addUnsubscribe}}>
			{children}
