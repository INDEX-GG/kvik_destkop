...
export const modifyGetPostsData = arr => {
    if (arr?.length > 0) {
        return arr?.map(item => {
            console.log('$$$',{ ...item, photo: JSON.parse(item.photo).photos})
            console.log('photo',JSON.parse(item.photo).photos.photos)
            console.log('object ?',typeof JSON.parse(item.photo) === 'object' && JSON.parse(item.photo) !== null)
            return {
                ...item,
