...
export const generateAdditionalFields = (obj) => {

    const arr = []

    for (const [key, value] in Object.entries(obj)) {
        if (key === 'title' || key === 'description' || key === 'price' || key === 'trade' || key === 'location'
            || key === 'bymessages' || key === 'byphone' || key === 'breed' || key === 'city'
            || key === 'coordinates' || key === 'alias' || key === 'user_id') {
            return;
        }
