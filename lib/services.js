...
export function phoneNumber(num) {
  if (num && num.length === 11) {
    const number = `+${num.substr(0, 1)} (${num.substr(1, 3)}) ${num.substr(
      4,
      3
    )} - ${num.substr(6, 2)} - ${num.substr(8, 2)}`;
    return number;
  } else {
    return String(num);
  }
...
export function phoneNumberWithPlus(num) {
  const isPhoneWithPlus = num.includes("+");
  if ((num && num.length === 11) || (isPhoneWithPlus && num.length === 12)) {
    const number = `+${num.substr(1, 1)} (${num.substr(2, 3)}) ${num.substr(
      5,
      3
    )}-${num.substr(8, 2)}-${num.substr(10, 2)}`;
    return number;
  } else {
    return String(num);
  }
          ...
          photo:
            item.photo !== null
              ? JSON.parse(item.photo).photos?.photos !== undefined
                ? JSON.parse(item.photo).photos.photos?.map(
                    (item) => `${STATIC_URL}/${item}`
                  )
                : JSON.parse(item.photo).photos?.map(
                    (item) => `${STATIC_URL}/${item}`
                  )
              : null,
        };
...
export const generateDataArr = (arr) => {
  const dataArr = arr.map((offer) => {
    return {
      ...offer,
      photo: JSON.parse(offer.photo)?.photos.map(
        (img) => `${STATIC_URL}/${img}`
      ),
    };
  });
...
export const generateSearchName = (location, city = false) => {
  ...
  if (data) {
    ...
    if (city !== "" || settlement !== "") {
      fullCity += `${data.country_iso_code}$${
        data.region_iso_code
      }${area}${city}${settlement}`;
    } else {
      if (!city) {
        fullCity = undefined;
      }
...
export const generateCityArr = (action, searchCity, excludesLength) => {
  ...
  if (action === "includes") {
    return sarchCityArr
      .splice(0, sarchCityArr.length - excludesLength + 1)
      .join("$");
  }

  if (action === "excludes") {
    return excludesLength > 1
      ? sarchCityArr
          .splice(0, sarchCityArr.length - excludesLength + 2)
          .join("$")
      : "";
  }
};
