...
export const scrollAds = async (scrollData) => {
	...
	const {id, isAuth, page, limit, data, setData, setLastIdAds, setLimitRanderPage, setPage, sort, region_includes, region_excludes} = scrollData

	if (isAuth) {
		if (page > 1 && id) {
		await getDataByPost('/api/getPostsPortion', {'page_limit': limit, 'page': page, 'user_id': id, sort, region_includes, region_excludes}).then(r => {
		...
		if (page > 1) {
			await getDataByPost('/api/getPostsPortion', {'page_limit': limit, 'page': page, 'user_id': id ? id : 0, sort, region_includes, region_excludes}).then(r => {
			setLimitRanderPage(0)
...
export const firstAds = (data) => {
	const {id,isAuth, page, limit, setData, setLastIdAds, sort, region_includes, region_excludes} = data
